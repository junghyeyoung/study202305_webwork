...
public class FortuneServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		/*
		 * 	HttpServletRequest 객체의 setAttribute( key. value ) 메소드를 이용해서 응답에 필요한 데이터를 담아둘수가 있다.
		 * 	담은 데이터는 응답하기 전까지 유효하며 응갑을 마친 이후에는 없어지는 1회성 데이터이다.
		 * 	담은 데이터를 얻어내는 방법은 HttpServletRequest 객체의 getAttribute(key) 메소드를 활용하면 된다.
		 * 
		 *  단, 담을 때는 object type 으로 받아주기 때문에 어떤 type 이든 담을 수 있지만
		 *  얻어낼 때도 역시 object type 으로 return 되기 때문에 원래 type 을 기억하고 있다가 casting 하는 것이 필요하다.
		 *  
		...
		//응답을 여기서 하지않고 jsp 페이지로 응답을 위임할 수 있다.
